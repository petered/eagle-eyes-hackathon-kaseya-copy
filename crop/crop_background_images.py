...
from hackathon.data_utils.data_loading import AnnotatedImageDataLoader, BGRImageArray, AnnotatedImage, Annotation
...
def is_detection_in_annotation(detection: Detection, annotation: Annotation) -> bool:
    di, dj, _, _ = detection.ijhw_box
    ai, aj, ah, aw = annotation.ijhw_box
    return (ai-ah/2 < di < ai + ah/2) and (aj-aw//2 < dj < aj + aw//2)
...
class MyModel(IDetectionModel):
    ...
    n_detections: int = 10  # Number of detections to return
    box_size: int = 100  # Somewhat arbitrary - only affects display, not score
    debug: bool = False
    ...
    def detect(self, image: BGRImageArray) -> Sequence[Detection]:
        ...
        image_tensor = tf.squeeze(background_subtracted_image)
        ...
        # detection_list = [Detection(ijhw_box=(i, j, self.box_size, self.box_size), score=score,) for (i, j), score in zip(local_maxima_ij.numpy(), maxima.numpy())]
        # for false_windows in detection_list:
        #     top = max(0, false_windows.ijhw_box[0] - 50)
        #     left = max(0,false_windows.ijhw_box[1] - 50)
        #     cropped_image = image.crop((left, top, left + 100, top + 100))
        #     infer.trainer.predict()

        # print(detection_list[0].ijhw_box)
...
def get_false_postives_cropped(
    annotated_image: AnnotatedImage,
    detector: IDetectionModel,

) -> Sequence[Tuple[int, int]]:

    detections = detector.detect(annotated_image.image)
    ground_truth: List[Annotation] = annotated_image.annotations
    
    false_detections = [d for d in detections if not any(is_detection_in_annotation(d, a) for a in ground_truth)]
    
    # false_detections = []
    # for d in detections:
    #     if any(is_detection_in_annotation(d, a) for a in ground_truth):
    #         false_detections.append(d)

    false_detection_ijs = [d.ijhw_box[:2] for d in false_detections]
    
    return false_detection_ijs


# def save_false_positive_crops(annotated_image: AnnotatedImage):
    
#     detector = MyModel()

#     false_points = get_false_postives_cropped(
#         annotated_image, detector
#     )

    
def save_all_false_positive_crops(
    ...
    detector = MyModel()
    index = 0
    for annotated_image in data_loader:
        false_points = get_false_postives_cropped(annotated_image, detector)       
        rgb_image = annotated_image.image[:, :, ::-1]
        for false_point_item in false_points:
            
            top = max(0, false_point_item[0] - 50)
            left = max(0,false_point_item[1] - 50)
            image = Image.fromarray(rgb_image,"RGB")
            cropped_image = image.crop((left, top, left + 100, top + 100))
            index_s = str(index)
            index_name = index_s + ".jpg"
            save_path = os.path.join(destination_dir_path, index_name)
            print(save_path)
...
def main():
    ...
    save_all_false_positive_crops("/home/kaseya/Downloads/eagle_eyes_hackathon_dataset/false", data_loader)
    print("done cropping")
